        ...
        Card {
            VStack(alignment: .leading, spacing: 12) {
                ...
                if !sortedWorkouts.isEmpty {
                    ForEach(sortedWorkouts.prefix(5), id: \.id) { workout in
                        VStack(alignment: .leading, spacing: 8) {
                            HStack(spacing: -2) {
                                VStack(alignment: .leading, spacing: 4) {
                            ...
                            if expanded.contains(workout.id) {
                                ExpandedWorkoutExerciseRows(workout: workout, isExpanded: expanded.contains(workout.id))
                            }
                        }
                        .padding(.vertical, 6)
                    }
...
struct ExpandedWorkoutExerciseRows: View {
    @EnvironmentObject private var store: WorkoutStore
    ...
    var body: some View {
        VStack(spacing: 8) {
            ForEach(workout.exercises) { wExercise in
                Card(hPadding: 1, vPadding: 1, contentPadding: 3, cRadius: 10) {
